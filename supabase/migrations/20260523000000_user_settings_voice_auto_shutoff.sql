-- Voice input auto-shutoff: seconds of silence before the prompt voice
-- button stops listening. 0 (default) keeps manual-only behavior.
alter table public.user_settings
  add column if not exists voice_auto_shutoff_seconds integer not null default 0;
...
begin
  ...
  values (
    ...
    coalesce(p_voice_auto_shutoff_seconds, 0)
  )
  on conflict (user_id)
  do update set
