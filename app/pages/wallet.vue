...
// One-page layout: sections stack in a single scroll column (no inner tabs).
// openWorkflow() scrolls to the Budgets section instead of switching tabs.
const budgetsSectionEl = ref<HTMLElement | null>(null)
...
function openWorkflow(id: string) {
  ...
  budgetsSectionEl.value?.scrollIntoView({ behavior: 'smooth', block: 'start' })
}
...
function formatDate(dateStr: string, detailed = false) {
  ...
  if (diff === 1) return t('vault.wallet.dateYesterday')
  if (diff < 7) return t('vault.wallet.dateDaysAgo', { n: diff })
  return d.toLocaleDateString([], { month: 'short', day: 'numeric' })
}
    ...
    <TabPanel ref="walletTabPanelRef" class="min-h-0 min-w-0 flex-1">
      ...
      <div v-else-if="showWalletUI" class="flex min-w-0 flex-col">
          ...
          <div
            ...
            class="mx-auto flex w-full max-w-[940px] flex-col gap-2 px-4 pt-4 sm:px-6"
          >
            <div
              v-if="topUpSuccess"
          ...
          <!-- One-page column: hero · activity · by-workflow · transactions · budgets · policies -->
          <div class="flex min-w-0 flex-1 flex-col">
            <div class="mx-auto flex w-full min-w-0 max-w-[940px] flex-col gap-10 px-4 py-7 sm:px-6 sm:py-9">
              <!-- Balance hero + quiet stat row -->
              <div>
                <div class="flex flex-wrap items-end justify-between gap-x-6 gap-y-4 pb-7">
                  <div class="min-w-0">
                    <span class="text-[10px] font-semibold uppercase tracking-[0.14em] text-neutral-400">{{ t('vault.wallet.overview.availableBalance') }}</span>
                    <div class="mt-2.5">
                      <span
                        v-if="balanceLoading"
                        class="block h-14 w-44 animate-pulse rounded-xl bg-neutral-200 sm:h-16 sm:w-56"
                        role="status"
                        :aria-label="t('common.loading')"
                      />
                      <span v-else class="block font-mono text-6xl font-bold leading-[0.95] tracking-tight text-neutral-950 sm:text-7xl">{{ balanceDisplay }}</span>
                    </div>
                    <p v-if="autoTopUpEnabled" class="mt-3.5 flex items-center gap-2 text-[13px] text-neutral-500">
                      <span class="size-1.5 shrink-0 rounded-full bg-green-600" aria-hidden="true" />
                      {{ t('vault.wallet.overview.autoTopUpSummary', { amount: `$${autoTopUpAmount}`, threshold: `$${autoTopUpThreshold}` }) }}
                    </p>
                  </div>
                  <div class="flex items-center gap-2">
                    <button
                      type="button"
                      class="flex items-center gap-1.5 rounded-full border px-3.5 py-2 text-xs font-semibold transition-colors"
                    ...
                    <button
                      type="button"
                      class="flex items-center gap-1.5 rounded-full bg-neutral-950 px-3.5 py-2 text-xs font-semibold text-white transition-opacity hover:opacity-90"
                      @click="openTopUp"
                    >
                ...
                <!-- Quiet stat row (pulled up from the activity card) -->
                <div class="grid grid-cols-2 border-t border-neutral-200/70 sm:grid-cols-4">
                  <div class="py-5 pr-4">
                    <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-neutral-400">{{ t('vault.wallet.activity.spentN', { n: chartRange }) }}</div>
                    <div class="mt-1.5 font-mono text-xl font-bold tracking-tight text-neutral-950">{{ formatCents(totalSpent) }}</div>
                  </div>
                  <div class="py-5 pr-4 sm:border-l sm:border-neutral-200/60 sm:pl-5">
                    <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-neutral-400">{{ t('vault.wallet.activity.toppedN', { n: chartRange }) }}</div>
                    <div class="mt-1.5 font-mono text-xl font-bold tracking-tight text-neutral-950">{{ formatCents(totalTopped) }}</div>
                  </div>
                  <div class="border-t border-neutral-200/60 py-5 pr-4 sm:border-t-0 sm:border-l sm:pl-5">
                    <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-neutral-400">{{ t('vault.wallet.activity.dailyAverage') }}</div>
                    <div class="mt-1.5 font-mono text-xl font-bold tracking-tight text-neutral-950">{{ formatCents(averageDaily) }}</div>
                  </div>
                  <div class="border-t border-neutral-200/60 py-5 pr-4 sm:border-t-0 sm:border-l sm:pl-5">
                    <div class="text-[10px] font-semibold uppercase tracking-[0.12em] text-neutral-400">{{ t('vault.wallet.activity.activeDays') }}</div>
                    <div class="mt-1.5 font-mono text-xl font-bold tracking-tight text-neutral-950">{{ daysWithSpend }} <span class="text-xs font-normal text-neutral-400">/ {{ chartRange }}</span></div>
                  </div>
                </div>
              </div>

              <!-- Activity chart (flat, hairline baseline) -->
              <section>
                ...
                <div class="relative mt-5">
                  ...
                  <div class="flex h-40 items-end gap-[2px] border-b border-neutral-200/70 pl-1 pr-12">
                    <button
              ...
              <!-- Spending by workflow (flat list) -->
              <section>
                ...
                <div v-else class="border-t border-neutral-200/70">
                  <button
                    v-for="w in visibleWorkflowRows"
                    :key="w.sessionId"
                    class="grid w-full grid-cols-[minmax(0,160px)_1fr_auto] items-center gap-4 border-b border-neutral-200/60 px-1.5 py-3 text-left transition-colors hover:bg-neutral-50"
            ...
            <section class="flex flex-col gap-4">
              <h3 class="text-sm font-semibold text-neutral-950">{{ t('vault.wallet.sections.transactions') }}</h3>
              ...
              <div class="overflow-x-auto">
                  <table class="w-full min-w-[780px] border-collapse">
                    <thead>
                      <tr class="border-b border-neutral-200/70 text-left text-[10px] font-semibold uppercase tracking-widest text-neutral-400">
            ...
            <!-- Budgets (per-workflow allocations, master–detail) -->
            <section ref="budgetsSectionEl" class="flex scroll-mt-6 flex-col gap-4">
              <h3 class="text-sm font-semibold text-neutral-950">{{ t('vault.wallet.sections.budgets') }}</h3>
            ...
            <!-- Policies -->
            <section class="flex flex-col gap-4">
              <h3 class="text-sm font-semibold text-neutral-950">{{ t('vault.wallet.sections.policies') }}</h3>
