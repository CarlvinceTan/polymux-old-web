...
// Mock storage files - in a real app these would come from an API/store
...
const filteredFiles = computed(() => {
  ...
  if (!term.trim()) return storageFiles.value
  return storageFiles.value.filter((f) => fuzzyMatch(f.name, term))
})
