...
//
// Parallel to — not shared with — `useLocalFileStorage`, which is scoped to
// workflow artifacts. Different DB to avoid version coordination with that
// composable; different metadata shape to avoid conflating concepts.
//
// Keys:
//  - IDB: compound string `{workspaceId}:{fileId}` → metadata row.
//  - OPFS: `workspace_files/{workspaceId}/{fileId}` → blob bytes.

export interface WorkspaceLocalFileMeta {
...
export function useWorkspaceLocalFiles() {
  async function write(
    ...
    const row: WorkspaceLocalFileMeta = {
  ...
  async function read(workspaceId: string, fileId: string): Promise<{ blob: Blob; meta: WorkspaceLocalFileMeta } | null> {
    const blob = await readBytes(workspaceId, fileId)
    if (!blob) return null
    const meta = await txMeta<WorkspaceLocalFileMeta>('readonly', store => store.get(compoundKey(workspaceId, fileId)))
    if (!meta) {
  ...
  async function listWorkspace(workspaceId: string): Promise<WorkspaceLocalFileMeta[]> {
    const db = await openDb()
    return new Promise<WorkspaceLocalFileMeta[]>((resolve, reject) => {
      const tx = db.transaction(META_STORE, 'readonly')
      const store = tx.objectStore(META_STORE)
      const index = store.index('by_workspace')
      const req = index.getAll(workspaceId)
      req.onsuccess = () => resolve((req.result ?? []) as WorkspaceLocalFileMeta[])
      req.onerror = () => reject(req.error)
    })
  }
