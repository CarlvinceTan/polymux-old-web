...
export function applyOp(graph: WorkflowGraph, op: WorkflowOp): ApplyResult | ApplyError {
  ...
  switch (op.kind) {
    ...
    case 'node_remove': {
      ...
      // Cascade-drop every wire incident on the removed node.
      next.wires = next.wires.filter(w => w.from_id !== op.node_id && w.to_id !== op.node_id)
      return { ok: true, graph: next }
    }
    case 'wire_add': {
      const wire: WorkflowWire = { ...op.wire }
      if (!wire.from_id || !wire.to_id) {
        return { ok: false, error: 'wire_add: from_id and to_id are required' }
      }
      if (findNodeIndex(next.nodes, wire.from_id) < 0) {
        return { ok: false, error: `wire_add: from_id ${wire.from_id} does not match any node` }
      }
      if (findNodeIndex(next.nodes, wire.to_id) < 0) {
