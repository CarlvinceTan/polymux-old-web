...
export function useSession(sessionId: Ref<string> | string) {
  ...
  function dispatch(type: string, payload: unknown) {
    ...
    for (const h of set) {
      try {
        ...
        console.error(`[useSession] handler error for "${type}"`, err)
      }
    }
  }
  ...
  async function connect() {
    ...
    socket.onmessage = (event: MessageEvent) => {
      ...
      if (event.data instanceof ArrayBuffer) {
        for (const h of binaryHandlers) {
          try { h(event.data) } catch (err) { console.error('[useSession] binary handler error', err) }
        }
        return
      }
      ...
      try {
        ...
        console.warn('[useSession] failed to parse message', event.data)
        return
      }
    ...
    socket.onerror = (err) => {
      console.error('[useSession] WebSocket error', err)
