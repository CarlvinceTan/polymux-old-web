      ...
      >
        ...
        >
          ...
          >
            <div
              v-if="isOpen"
              class="w-full max-w-[420px] overflow-hidden rounded-2xl bg-white shadow-[0_8px_30px_rgba(0,0,0,0.12),0_2px_8px_rgba(0,0,0,0.08)] ring-1 ring-neutral-200"
              role="dialog"
              aria-modal="true"
              :aria-label="t('workflow.deleteTitle', { name: workflowName })"
              @click.stop
            >
              <div class="px-5 pt-5 pb-4">
                <h2 class="text-sm font-semibold text-neutral-900">
                  {{ t('workflow.deleteTitle', { name: workflowName }) }}
                </h2>
                <p class="mt-1.5 text-xs leading-relaxed text-neutral-600">
                  {{ t('workflow.deleteBody') }}
                </p>
                <ul class="mt-2.5 list-disc space-y-1 pl-5 text-xs leading-relaxed text-neutral-600">
                  <li>{{ t('workflow.deleteLossChat') }}</li>
                  <li>{{ t('workflow.deleteLossArtifacts') }}</li>
                  <li>{{ t('workflow.deleteLossRuns') }}</li>
                  <li>{{ t('workflow.deleteLossSchedule') }}</li>
                </ul>
                <p class="mt-2.5 text-xs leading-relaxed text-neutral-600">
                  {{ t('workflow.deleteNotice') }}
                </p>
              </div>
              <div class="flex justify-end gap-2 border-t border-neutral-100 px-5 py-3.5">
                ...
                <button
                  ...
                  @click="emit('confirm')"
                >
                  {{ submitting ? t('workflow.deleting') : t('workflow.deleteConfirm') }}
                </button>
              </div>
            </div>
          </Transition>
